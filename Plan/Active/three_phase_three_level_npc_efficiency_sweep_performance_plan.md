...
- 检查并锁定 GUI Efficiency Sweep 控制器继续使用当前 `design_report`、当前 NPC 插件和当前 `run_context`；控制器不会重新执行设计、器件选型或磁性搜索。
- 新增 NPC GUI 控制器专项测试：使用当前设计报告执行一个负载点，验证候选器件、拓扑候选、运行 ID 和效率扫描输出目录保持当前运行范围，效率结果回写 `AppStateStore`。
- 保持现有 GUI 输入窗口、负载扫描和 PF 扫描契约不变；效率图、损耗分解、结构化结果和硬件概览继续由已有 GUI 刷新链路加载。
- 最终验证：NPC 步骤、合同和打包 GUI 运行时测试 `46 passed`；`py_compile`、`git diff --check` 通过。测试临时目录为 `pytest_temp/npc-efficiency-step6-*`，设计结果和性能记录未提交到 Git。
- 最终代表性验证使用固定器件、磁性件和上下分裂直流链电容：两点负载扫描约 `17.19 s`（约 `8.60 s/点`），生成 `20` 个 PF 点，两个负载点各包含 `4800` 个实际开关事件；效率约 `99.60%`、`99.57%`。相较第一步约 `723.58 s` 的 40 点估算，当前同口径点耗时约降低 `52.5%`，并已通过前五步的精确事件电流、阻断电压、软开通和 SiC 反向恢复为零契约。
- 第六步测试提交和计划/日志回执提交均已独立推送到 `origin/codex/npc-output-run-isolation-step1`；第 1 至第 6 步均已完成，本计划关闭。
