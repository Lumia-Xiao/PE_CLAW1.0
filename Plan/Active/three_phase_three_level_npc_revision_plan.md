...
状态：已完成。实现提交 `f87f9da` 已推送至 `origin/codex/npc-output-run-isolation-step1`；计划与 ChangeLog 文档提交待本次文档提交后填写。

完成记录：新增 NPC 专用输出电感/滤波器复核。审计固定保留 E 80/38/20、FT-3M、34 匝、6 并绕的历史基线（271.102 uH、0.468 T、4.8096 W/只），并对同磁芯、同材料、34 匝、2 并绕的 Litz 600x0.08 候选给出 3.9393 W/只的可复算对比。五个工作点（额定、最大母线、最低 PF、110% 过载、最高环境温度）计算了 Bsat(100 C) 饱和裕量、热态铜阻、趋肤效应、绕组邻近效应、气隙边缘磁通修正、铜损/磁芯损耗和热点估计；同时完成三相 LC 谐振、串联 RC 阻尼、控制环路带宽比和电流纹波审计。结果写入当前运行目录 `inductor_design/npc_output_filter_audit.json` 与 `npc_inductor_operating_cases.csv`，并通过结构化报告暴露。

默认审计结论为“有条件通过”：基线和 Litz 参考构型在解析工况下均保留饱和裕量，Litz 铜损更低但绕组体积更大；FT-3M 规范化记录有频率范围、Bsat 温度点和源记录，但没有 DC 偏磁损耗图或温度损耗系数，输出滤波器的 2.2 uF/相电容、控制带宽和电网阻抗仍需阻抗扫描与样机测试确认。当前磁性搜索得到的最新 535.826 uH 候选不替代计划要求的 271.1 uH 历史基线，二者均保留在运行产物中供选型比较。

验证：步骤 8 专项 `3 passed`；NPC 合同、步骤 5 损耗、步骤 6 热设计、步骤 7 电容和运行隔离回归 `32 passed`；`python -B -m compileall -q src tests`、`git diff --check` 通过。分支为 `codex/npc-output-run-isolation-step1`，实现 commit 为 `f87f9da2868c8d27cf6379600f1a8b84071b2a95`，push 成功；本次文档提交完成后将再次核验本地 HEAD 与远端一致。
...
状态：已执行，设计验收结论为“不通过”；第九步实现提交 `f26b066` 已推送至
`origin/codex/npc-output-run-isolation-step1`。由于默认设计在最低母线电压下调制度超限，计划继续保留在
`Plan/Active`，不得将本步骤标记为“设计通过”或将整个计划归档。
...
- 修改范围：新增 NPC 第九步完整流水线、72 点母线/负载/PF 验证矩阵、交叉阶段检查、运行目录产物完整性检查、结构化报告 `system_validation` 字段，以及 NPC Hardware Overview 对缺失热推荐结果的兼容处理。
- 验证矩阵：母线最小/额定/最大 `650/700/750 V`，负载 `5%/25%/50%/75%/100%/110%`，PF `-1/-0.8/0.8/1`，共 `72` 点；同时复用步骤 5 至步骤 8 的损耗、热、电容和输出滤波结果。
- 运行结果：结果目录为 `outputs/20260902_step9_npc_system_validation`；`manifest.json`、设计请求、半导体热设计、电容、电感/滤波、效率、硬件总览和验证 JSON/CSV 均位于同一运行根目录。
- 设计结论：`fail`。最低母线 `650 V` 时调制度为 `1.0049188688`，超过 `1.0` 限值；其余已建模硬检查通过，验证矩阵为 `72/72`，分裂母线中点代理偏差最大为 `0.8%`。
- 未验证风险：短路保护及保护时序、双脉冲/母排寄生过压、传导与辐射 EMI；这些项目已在最终结果中标记为 `unverified`，没有被当作硬件验证结论。
