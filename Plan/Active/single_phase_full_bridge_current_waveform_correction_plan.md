...
- 实现提交：`da0c64a` (`feat: add full-bridge average-current feedback`)
- 变更：逐开关周期计算实际平均电流，最多 3 次电压修正，按 `Vnew = Vold - 2L * error / Tsw` 限幅
- 记录：参考/实际平均电流、平均误差、修正次数、饱和标志、修正前后目标电压、收敛状态和最大误差
- 验证：`python -m pytest -q tests/test_single_phase_full_bridge_switching_loss.py tests/test_single_phase_full_bridge_current_baseline.py tests/test_dc_ac_single_phase_full_bridge_contract.py` -> `27 passed`
- 验证：`git diff --check` -> passed
