...
### 第 6 步执行回执（2026-09-07）

- 状态：已完成。
- 器件页、损耗页和硬件概览页统一采用 NPC 当前工况损耗完整性判定；当前工况三类角色完整时显示当前工况，否则统一显示设计点并给出 warning。
- 器件页明确显示单物理器件损耗、角色总损耗和半导体方案总损耗；损耗页和硬件概览复用共享 NPC 角色聚合，避免重复乘数量。
- 硬件概览补充组级/角色级 `loss_scope`、损耗 basis、当前工况完整性 metadata 和 notes。
- NPC 定向验证：新增页面一致性和不完整当前工况回退测试；`10 passed`，`compileall` 和 `git diff --check` 通过。
- 未修改器件选型规则、开关事件损耗模型和其他拓扑。
- 实现提交和 push 回执：`d8ace6f`，已推送至 `origin/codex/npc-output-run-isolation-step1`。
