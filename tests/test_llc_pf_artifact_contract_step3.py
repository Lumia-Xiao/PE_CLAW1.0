...
def _workspace_tmp(name: str) -> Path:
    path = Path(__file__).resolve().parents[1] / f".pytest-tmp-{name}-{uuid4().hex}"
    path.mkdir()
    return path


def test_complete_role_contracts_are_available_and_hashed() -> None:
    root = _workspace_tmp("llc-step3-contract")
    try:
...
def test_missing_empty_and_wrong_role_artifacts_are_blocked() -> None:
    root = _workspace_tmp("llc-step3-missing")
    try:
...
def test_contract_rejects_outside_run_and_identity_mismatch() -> None:
    root = _workspace_tmp("llc-step3-identity")
    outside_root = root.parent / f"llc-outside-{uuid4().hex}"
    outside_root.mkdir()
...
def test_structured_output_exposes_llc_pf_artifacts_only_for_llc() -> None:
    root = _workspace_tmp("llc-step3-structured")
    try:
...
def test_pf_view_prefers_role_contract_paths() -> None:
    root = _workspace_tmp("llc-step3-view")
    try:
        report = build_baseline_report()
        context = LlcRunContext.create(TOPOLOGY, {"fixture": "step3"}, output_root=root)
        contracts = _contracts(root, run_id=context.run_id)
