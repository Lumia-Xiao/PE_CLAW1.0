...
def test_npc_thermal_has_five_scenarios_three_roles_and_corrected_loss_basis(tmp_path: Path) -> None:
    ...
    # The retired peak/sample-based model's 80 W floor is not an event-model contract.
    for scenario in thermal.npc_scenarios:
        assert scenario.total_semiconductor_loss_w == pytest.approx(sum(
            role.physical_device_count * role.per_device_loss_w for role in scenario.roles
        ))
        for role in scenario.roles:
            expected_tj = (
                scenario.ambient_temp_c
                + scenario.total_semiconductor_loss_w * scenario.thermal_coupling_factor * scenario.selected_sink_rth_k_per_w
                + role.per_device_loss_w * (role.rth_jc_k_per_w + role.rth_cs_k_per_w)
            )
            assert role.junction_temp_c == pytest.approx(expected_tj)
    ...
    refreshed = run_device_operating_point_refresh(
        replace(report, operating_point=OperatingPoint(vin_v=700.0, load_ratio=1.0, power_factor=1.0)),
        plugin=build_default_registry().get_plugin(report.spec.topology_id),
    )
    rated = next(item for item in thermal.npc_scenarios if item.scenario_id == "rated")
    for role in rated.roles:
        current_loss = next(loss for loss in refreshed.device.current_operating_losses.values() if loss.role == role.role)
        assert role.per_device_loss_w == pytest.approx(current_loss.p_total_W)
