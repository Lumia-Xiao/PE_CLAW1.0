...
def test_npc_waveform_uses_user_switching_frequency_and_records_events() -> None:
    ...
    for role in ("outer_switch", "inner_switch"):
        for edge in ("turn_on", "turn_off"):
            expected = sorted(
                (float(event["signed_current_A"]), float(event["blocking_voltage_V"]))
                for event in events if event["role"] == role and event["event_type"] == edge
            )
            actual = sorted(zip(
                roles[role][f"{edge}_event_currents_a"],
                roles[role][f"{edge}_event_voltages_v"],
                strict=True,
            ))
