...
def design_buck_task(self,job_id):
    ...
    try:
        store.update(job_id,progress=15,stage='topology'); result=run_buck_design(request); artifacts=save_result(job_id,result); payload=result.model_copy(update={'job_id':job_id,'artifacts':artifacts}); store.update(job_id,status='succeeded',progress=100,stage='finalize',finished_at=datetime.now(timezone.utc),result_json=payload.model_dump(mode='json')); return payload.model_dump(mode='json')
    except Exception:
