...
def _npc_thermal_scenario(report, basis, devices, definition, topology_plugin) -> NpcThermalScenarioResult:
    ...
    waveform = call_with_report_run(
        report, topology_plugin.generate_waveforms, report.candidate, operating_point=operating_point
    )
    scenario_report = replace(
        report,
        operating_point=operating_point,
        waveform=waveform,
        stress=call_with_report_run(report, topology_plugin.extract_stress, report.candidate, waveform_set=waveform),
    )
...
def _npc_scenario_losses(report, current_case, devices, device_result):
    if current_case is None:
        return {}
    from .run_device_pipeline import _evaluate_role_loss, scale_switch_stress_for_parallel
    ...
    for stress in current_case.stresses:
        ...
        result[stress.role] = {
            ...
            "result": _evaluate_role_loss(
                device, report, scaled_stress, report.operating_point, parallel_count=parallel_count
            ),
        }
    return result
