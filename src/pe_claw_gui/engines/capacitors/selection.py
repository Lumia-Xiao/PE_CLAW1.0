...
from .pareto import apply_representative_labels, choose_margin_aware_recommended_capacitor, extract_pareto_front
...
DC_LINK_APPLICATION_CATEGORIES = {"dc_link"}
...
def select_capacitor_bank(
    ...
    *,
    ...
    voltage_filtered = [
        candidate for candidate in application_filtered if candidate.voltage_rating_dc_v >= request.voltage_margin * request.dc_voltage_v
    ]
    rough_filtered = [
        candidate for candidate in voltage_filtered if _passes_static_prefilter(candidate) and _passes_nmax_prefilter(request, stats, candidate, max_parallel_count)
    ]
    dominance_filtered = _dominance_prefilter(rough_filtered) if optimize_parallel_counts else rough_filtered
    nmin_feasible_plans = _build_candidate_parallel_plans(request, stats, dominance_filtered, max_parallel_count)
    if optimize_parallel_counts:
        ...
        evaluation_counts = {id(plan.candidate): range(1, max_parallel_count + 1) for plan in candidate_plans}
    for plan in candidate_plans:
        for parallel_count in evaluation_counts.get(id(plan.candidate), ()):
            entries.append(evaluate_capacitor_bank_from_stats(request, stats, plan.candidate, parallel_count))
    ...
    recommendation = choose_margin_aware_recommended_capacitor(pareto_front, feasible)
    (
        feasible,
        pareto_front,
        min_volume,
    ...
    diagnostics = {
        ...
        "exhaustive_bank_evaluations": len(rough_filtered) * max_parallel_count,
        "detailed_bank_evaluations": len(entries),
        "avoided_bank_evaluations": max(len(rough_filtered) * max_parallel_count - len(entries), 0),
    ...
    notes = [
        "Capacitor selection uses DC voltage rating as the hard voltage filter for this DC-DC first pass.",
    ...
    if not feasible:
        warnings.append(f"No feasible {request.side} capacitor candidate was found up to {max_parallel_count} parallel parts.")
...
def evaluate_capacitor_bank(
    ...
    """Evaluate one candidate at one parallel count."""

    if parallel_count <= 0:
        raise ValueError("parallel_count must be positive.")

    stats = prepare_capacitor_waveform_stats(request)
    return evaluate_capacitor_bank_from_stats(request, stats, candidate, parallel_count)
...
def evaluate_capacitor_bank_from_stats(
    ...
    """Evaluate one candidate at one parallel count using prepared waveform stats."""

    if parallel_count <= 0:
        raise ValueError("parallel_count must be positive.")

    equivalent_capacitance_f = parallel_count * candidate.capacitance_f
    equivalent_rs_ohm = candidate.rs_ohm / parallel_count
    equivalent_esl_h = candidate.esl_h / parallel_count
    ...
    p_total_per_cap_w = p_total_w / parallel_count
    ...
    dvdt_required_v_per_us = _dvdt_required_v_per_us(stats.time_s, stats.q_values_c, equivalent_capacitance_f)

    voltage_margin_ratio = _safe_ratio(candidate.voltage_rating_dc_v, request.dc_voltage_v)
    ...
    dvdt_margin_ratio = _safe_ratio(candidate.dvdt_v_per_us, dvdt_required_v_per_us)
    total_volume_cm3 = parallel_count * _candidate_volume_cm3(candidate)

    rejection_reasons: list[str] = []
    if candidate.voltage_rating_dc_v < request.voltage_margin * request.dc_voltage_v:
        rejection_reasons.append(
            f"DC voltage rating {candidate.voltage_rating_dc_v:.6g} V is below {request.voltage_margin:.3g}x {request.dc_voltage_v:.6g} V."
        )
    ...
    if candidate.ripple_voltage_limit_ratio is not None and ripple_total_pp_v > candidate.ripple_voltage_limit_ratio * candidate.voltage_rating_dc_v:
        rejection_reasons.append(
            f"Ripple {ripple_total_pp_v:.6g} Vpp exceeds datasheet limit "
            f"{candidate.ripple_voltage_limit_ratio:.3g}x VNDC."
        )
    ...
    if dvdt_required_v_per_us > candidate.dvdt_v_per_us:
        rejection_reasons.append(
            f"dV/dt {dvdt_required_v_per_us:.6g} V/us exceeds rating {candidate.dvdt_v_per_us:.6g} V/us."
        )
...
def _passes_nmax_prefilter(
    ...
    return _passes_monotonic_constraints(request, stats, candidate, max_parallel_count)
...
def estimate_min_feasible_parallel_count(
    ...
    if (
        ...
        or candidate.voltage_rating_dc_v < request.voltage_margin * request.dc_voltage_v
    ...
    if stats.ripple_allow_v <= 0.0:
        if ripple_numerator_v > 0.0:
            return None
    else:
        nmin = max(nmin, _ceil_ratio(ripple_numerator_v, stats.ripple_allow_v))
    if candidate.ripple_voltage_limit_ratio is not None:
        datasheet_ripple_allow_v = candidate.ripple_voltage_limit_ratio * candidate.voltage_rating_dc_v
        if datasheet_ripple_allow_v <= 0.0:
            return None
        nmin = max(nmin, _ceil_ratio(ripple_numerator_v, datasheet_ripple_allow_v))
    ...
    dvdt_at_one_v_per_us = _dvdt_required_v_per_us(stats.time_s, stats.q_values_c, candidate.capacitance_f)
    nmin = max(nmin, _ceil_ratio(dvdt_at_one_v_per_us, candidate.dvdt_v_per_us))
    if nmin > max_parallel_count:
        return None
    while nmin <= max_parallel_count and not _passes_monotonic_constraints(request, stats, candidate, nmin):
        nmin += 1
...
def _build_candidate_parallel_plans(
    ...
    for candidate in candidates:
        nmin = estimate_min_feasible_parallel_count(request, stats, candidate, max_parallel_count)
        if nmin is None:
            continue
        plans.append(
            _CandidateParallelPlan(
                candidate=candidate,
                nmin=nmin,
                estimated_loss_at_nmin_w=_estimated_total_loss_w(stats, candidate, nmin),
                estimated_loss_at_max_w=_estimated_total_loss_w(stats, candidate, max_parallel_count),
                volume_at_nmin_cm3=nmin * _candidate_volume_cm3(candidate),
            )
        )
    return plans
...
def _optimized_parallel_counts_by_candidate(
    ...
    if len(plans) <= SMALL_LIBRARY_EXHAUSTIVE_THRESHOLD:
        return {id(plan.candidate): tuple(range(1, max_parallel_count + 1)) for plan in plans}
    ...
    counts_by_candidate: dict[int, tuple[int, ...]] = {}
    for plan in plans:
        counts = {plan.nmin}
        if plan.nmin < max_parallel_count and _near_constraint_limit(request, stats, plan.candidate, plan.nmin, near_limit_utilization):
            counts.add(plan.nmin + 1)
        if id(plan.candidate) in max_parallel_candidate_ids:
            counts.add(max_parallel_count)
        counts_by_candidate[id(plan.candidate)] = tuple(sorted(counts))
    return counts_by_candidate


def _max_parallel_boundary_candidate_ids(request: CapacitorSizingRequest, plans: list[_CandidateParallelPlan]) -> set[int]:
    keep: set[int] = set()
    ...
    for plan in sorted(plans, key=lambda item: (item.volume_at_nmin_cm3, item.estimated_loss_at_nmin_w, item.candidate.part_number))[
        ...
        keep.add(id(plan.candidate))
    keep.update(id(plan.candidate) for plan in _rough_candidate_pareto(plans))
    return keep
...
def _candidate_boundary_prefilter(
    ...
    keep_ids.update(id(plan.candidate) for plan in plans if plan.nmin == min_nmin)
    keep_ids.update(id(plan.candidate) for plan in _rough_candidate_pareto(plans))
    keep_ids.update(
        id(plan.candidate)
    ...
    kept = [
        plan
        for plan in plans
        if id(plan.candidate) in keep_ids
        or not (
            plan.candidate.capacitance_f * plan.nmin > OUTPUT_OVERCAP_REQUIRED_CAP_MULTIPLIER * required_capacitance_f
...
def _overcap_speed_prefilter(
    ...
    kept = [
        plan
        for plan in plans
        if plan.candidate.capacitance_f * plan.nmin <= cap_multiplier * required_capacitance_f
        or id(plan.candidate) in keep
    ]
    return kept if kept else plans
...
def _plan_dominates(a: _CandidateParallelPlan, b: _CandidateParallelPlan) -> bool:
    ...
    metrics = (
        ...
        (ac.voltage_rating_dc_v, bc.voltage_rating_dc_v, "higher"),
...
def _passes_monotonic_constraints(
    ...
    if parallel_count <= 0 or candidate.capacitance_f <= 0.0:
        return False
    equivalent_capacitance_f = parallel_count * candidate.capacitance_f
    equivalent_rs_ohm = candidate.rs_ohm / parallel_count
    ...
    if (
        ...
        and ripple_total_pp_v > candidate.ripple_voltage_limit_ratio * candidate.voltage_rating_dc_v
    ):
        return False
    ...
    p_total_per_cap_w = (p_joule_w + p_dielectric_w) / parallel_count
    if p_total_per_cap_w > candidate.pmax_w:
        return False
    delta_t_hotspot_c = p_total_per_cap_w * candidate.rth_hotspot_to_ambient_c_per_w
    ...
    if delta_t_hotspot_c > candidate.self_heating_limit_c:
        return False
    if _dvdt_required_v_per_us(stats.time_s, stats.q_values_c, equivalent_capacitance_f) > candidate.dvdt_v_per_us:
        return False
    return True
...
def _near_constraint_limit(
    ...
    equivalent_rs_ohm = candidate.rs_ohm / parallel_count
    ripple_total_pp_v = stats.q_swing_c / equivalent_capacitance_f + stats.i_pp_total_a * equivalent_rs_ohm
    p_total_per_cap_w = _estimated_total_loss_w(stats, candidate, parallel_count) / parallel_count
    delta_t_hotspot_c = p_total_per_cap_w * candidate.rth_hotspot_to_ambient_c_per_w
    allowed_hotspot_rise_c = candidate.hotspot_temp_max_c - request.ambient_temp_c
    utilizations = [
        ...
        _safe_ratio(_dvdt_required_v_per_us(stats.time_s, stats.q_values_c, equivalent_capacitance_f), candidate.dvdt_v_per_us),
    ]
    if candidate.ripple_voltage_limit_ratio is not None:
        utilizations.append(_safe_ratio(ripple_total_pp_v, candidate.ripple_voltage_limit_ratio * candidate.voltage_rating_dc_v))
    return max(utilizations) >= threshold


def _estimated_total_loss_w(stats: CapacitorWaveformStats, candidate: CapacitorCandidate, parallel_count: int) -> float:
    if parallel_count <= 0:
        return math.inf
    return _loss_numerator_w(stats, candidate) / parallel_count


def _loss_numerator_w(stats: CapacitorWaveformStats, candidate: CapacitorCandidate) -> float:
    if candidate.capacitance_f <= 0.0:
        return math.inf
    return (stats.i_rms_total_a**2) * candidate.rs_ohm + (stats.i_rms_total_a**2) * candidate.tan_delta_0 / (
        2.0 * math.pi * stats.dominant_frequency_hz * candidate.capacitance_f
    )
...
def _dedupe_entries(entries: list[CapacitorSelectionEntry]) -> list[CapacitorSelectionEntry]:
    ...
    for entry in entries:
        key = (entry.candidate.part_number, entry.parallel_count)
        if key in seen:
            continue
        seen.add(key)
...
def _series_basis_notes(candidates: tuple[CapacitorCandidate, ...]) -> list[str]:
    ...
    for candidate in candidates:
        ...
        if key in seen:
            continue
        seen.add(key)
        notes.append(f"{candidate.series} Rs/Irms basis: {candidate.irms_rating_basis}.")
    return notes
...
def _representative_series_basis_notes(entries: tuple[CapacitorSelectionEntry | None, ...]) -> list[str]:
    ...
    for entry in entries:
        ...
        if key in seen:
            continue
        seen.add(key)
        notes.append(f"{entry.candidate.series} Rs/Irms basis: {entry.candidate.irms_rating_basis}.")
    return notes
