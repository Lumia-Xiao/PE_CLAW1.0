"""Input form for the three-phase two-level voltage-source inverter."""
...
THREE_PHASE_TWO_LEVEL_NOTES = (
    "Runs a first-pass CCM fixed-frequency SPWM design for a balanced three-phase two-level VSI. "
    "Voltage input is line-line RMS; reported PF and sweep current are phase voltage to phase-current "
    "quantities, with current RMS based on line-line voltage.\n"
    "Hardware closure uses one selected main switch repeated across six bridge positions, an SxP "
    "DC-link electrolytic capacitor bank sized from the PWM-level switch-state current proxy, and one "
    "representative per-phase output inductor shown as three physical inductors in Hardware Overview.\n"
    "Efficiency sweep uses fixed selected hardware for load and PF diagnostics. The waveform and loss "
    "models remain first-pass: no SVPWM/DPWM, dead-time, Coss, parasitic ringing, harmonic-by-harmonic "
    "ESR, or FEA/CFD validation is included."
)


class ThreePhaseTwoLevelVoltageSourceInverterForm(BaseTopologyForm):
    """First-pass form for the three-phase two-level inverter."""
    ...
    def __init__(
        ...
        ttk.Label(
            notes,
            text=THREE_PHASE_TWO_LEVEL_NOTES,
            justify="left",
            wraplength=360,
    ...
    def update_from_report(self, report: DesignReport | None) -> None:
        """Enable efficiency sweep after fixed semiconductor hardware exists."""
        ...
        if self.run_efficiency_sweep_button is not None:
            state = (
                ...
                and (report.device.selected_devices or report.device.design_point_losses or report.device.scheme_results)
                else "disabled"
            )
