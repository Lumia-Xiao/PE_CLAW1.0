...
class WaveformView(ttk.Frame):
    ...
    def render(self, report: DesignReport | None) -> None:
        ...
        if report.spec.topology_id == "single_phase_full_bridge_inverter" and isinstance(tcm, dict):
            self._render_single_phase_tcm_waveforms(report, tcm)
            return
    ...
    def _render_single_phase_tcm_waveforms(self, report: DesignReport, data: dict[str, object]) -> None:
        ...
        line_period_s = float(report.waveform.time_span_s)
        for axis in axes:
            axis.set_xlim(0.0, line_period_s * 1e3)
        self._finish_inverter_plot(report, axes, "Vac", "single-phase TCM detail; one complete line cycle")
