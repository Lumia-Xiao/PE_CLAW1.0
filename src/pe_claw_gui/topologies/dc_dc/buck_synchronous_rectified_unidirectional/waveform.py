"""Waveform generation for the synchronous Buck topology."""
...
from .mode import BuckSrOperatingState, build_operating_state


def _sample_period(
    candidate: TopologyCandidate,
    state: BuckSrOperatingState,
    samples_per_period: int,
) -> tuple[list[float], list[float], list[float], list[float], list[float]]:
    """Sample one switching period of the synchronous Buck waveforms."""
    ...
    high_side_current_a: list[float] = []
    low_side_current_a: list[float] = []
    inductor_voltage_v: list[float] = []

    for tau in tau_values:
        if tau < on_time:
            current = state.il_min + (state.vin - state.vout) / candidate.inductance_h * tau
            switch_node_voltage_v.append(state.vin)
            high_side_current_a.append(current)
            low_side_current_a.append(0.0)
            inductor_voltage_v.append(state.vin - state.vout)
        else:
            off_tau = tau - on_time
            current = state.il_max - state.vout / candidate.inductance_h * off_tau
            switch_node_voltage_v.append(0.0)
            high_side_current_a.append(0.0)
            low_side_current_a.append(current)
            inductor_voltage_v.append(-state.vout)
...
def generate_waveforms(
    ...
    """Generate two switching periods for the synchronous Buck operating state."""
    if points < 2:
    ...
    period_switch_node, period_inductor_current, period_high_side_current, period_low_side_current, period_inductor_voltage = _sample_period(
        candidate,
        state,
        samples_per_period,
    )
    ...
    return WaveformSet(
        ...
        mode=state.mode,
        switch_current_a=_tile_period(period_high_side_current, points),
        diode_current_a=_tile_period(period_low_side_current, points),
        inductor_voltage_v=_tile_period(period_inductor_voltage, points),
        t_zero_current_s=None,
        notes=[
            "Generated from the synchronous Buck plugin over two switching periods in CCM.",
            "Inductor current may cross zero and become negative.",
            "For compatibility, diode_current_a stores the low-side synchronous MOSFET current.",
        ],
    )
