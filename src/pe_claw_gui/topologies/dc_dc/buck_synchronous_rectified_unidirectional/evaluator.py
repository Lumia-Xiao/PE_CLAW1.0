"""Synchronous Buck topology evaluation."""
...
def evaluate(
    ...
    """Evaluate the synchronous Buck candidate into a topology summary."""
    summary_lines = [
    ...
    if stress_result is not None:
        summary_lines.append(f"High-side RMS current = {(stress_result.switch.current_rms_a or 0.0):.6f} A")
        summary_lines.append(f"Low-side RMS current = {(stress_result.rectifier.current_rms_a or 0.0):.6f} A")
    ...
    return TopologyResult(
        ...
        notes=[
            "Continuous-current synchronous Buck model.",
            "Negative inductor current is allowed.",
            "Low-side conduction is modeled as synchronous-switch conduction, not diode freewheel.",
        ],
    )
...
def build_report(
    ...
    """Build the runtime design report for the synchronous Buck plugin."""
    return DesignReport(
        spec=spec,
        candidate=candidate,
        ...
        notes=["Runtime report assembled by the synchronous Buck unidirectional plugin."],
    )
