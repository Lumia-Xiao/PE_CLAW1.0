"""Input handling for the synchronous Buck topology."""
...
BUCK_TOPOLOGY_ID = "buck_synchronous_rectified_unidirectional"
BUCK_DISPLAY_NAME = "Buck Synchronous Rectified Unidirectional"
BUCK_LEGACY_KEY = "Buck_SynchronousRectified_Unidirectional"


def build_default_inputs() -> dict[str, str]:
    """Return default raw inputs for the synchronous Buck topology."""
...
def build_spec(raw_input: Mapping[str, str]) -> TopologySpec:
    """Parse and validate raw GUI inputs into a synchronous Buck topology spec."""
    try:
        spec = TopologySpec(
            topology_id=BUCK_TOPOLOGY_ID,
            display_name=BUCK_DISPLAY_NAME,
            ...
            metadata=merge_semiconductor_filter_metadata(
                merge_ambient_metadata({"legacy_key": BUCK_LEGACY_KEY}, raw_input),
                raw_input,
            ),
        )
        ...
        raise ValueError("All synchronous Buck design inputs must be valid numbers.") from exc
