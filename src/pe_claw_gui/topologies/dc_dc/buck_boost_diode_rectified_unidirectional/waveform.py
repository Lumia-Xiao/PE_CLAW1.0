"""Waveform generation for the inverting diode Buck-Boost topology."""
...
from .mode import BuckBoostOperatingState, build_operating_state


def _sample_ccm_tau(state: BuckBoostOperatingState, inductance_h: float, tau: float) -> tuple[float, float, float, float, float]:
    ...
    off_tau = tau - on_time
    current = max(state.il_max - state.vout_mag / inductance_h * off_tau, state.il_min)
    return state.vin + state.vout_mag, current, 0.0, current, -state.vout_mag


def _sample_dcm_tau(state: BuckBoostOperatingState, inductance_h: float, tau: float) -> tuple[float, float, float, float, float]:
    on_time = state.duty * state.switching_period_s
    ...
    if tau < on_time + diode_time:
        off_tau = tau - on_time
        current = max((state.i_pk or 0.0) - state.vout_mag / inductance_h * off_tau, 0.0)
        return state.vin + state.vout_mag, current, 0.0, current, -state.vout_mag
...
def _sample_period(
    ...
    state: BuckBoostOperatingState,
    samples_per_period: int,
) -> tuple[list[float], list[float], list[float], list[float], list[float]]:
    """Sample one switching period of the inverting Buck-Boost waveforms."""
...
def _build_period_output_voltage(
    ...
    """Numerically integrate capacitor current to recover Buck-Boost output ripple."""
    ...
    return capacitor_current_a, [state.vout_mag + value for value in centered_ripple]
...
def generate_waveforms(
    ...
    """Generate two switching periods for the resolved Buck-Boost operating mode."""
    if points < 2:
    ...
    notes = [
        f"Generated from the inverting diode Buck-Boost plugin over two switching periods in {state.mode}.",
        "Output-voltage ripple is integrated numerically from capacitor current segmentation.",
        "The plotted output voltage is shown as magnitude; the physical output polarity is inverted.",
    ]
    if state.mode == "DCM":
