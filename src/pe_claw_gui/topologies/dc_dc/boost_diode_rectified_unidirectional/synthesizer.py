"""Diode Boost synthesis logic used by the runtime plugin."""
...
def synthesize(spec: TopologySpec) -> TopologyCandidate:
    """Synthesize the nominal diode Boost hardware design."""
    vin_nom = 0.5 * (spec.vin_min + spec.vin_max)
    fs_hz = spec.fs_khz * 1e3
    iout = spec.pout / spec.vout
    duty_nom = _clamp_duty(1.0 - vin_nom / spec.vout)
    ...
    r_crit_nom_ohm = 2.0 * inductance_h * fs_hz / max(duty_nom * (1.0 - duty_nom) ** 2, 1e-9)
