"""First-pass synthesis for the single-phase boost PFC topology."""
...
from .line_cycle import sample_boost_pfc_half_line_cycle
...
def synthesize(spec: TopologySpec) -> TopologyCandidate:
    """Build first-pass CCM boost PFC electrical estimates."""
    ...
    input_inductance_h = float(spec.metadata["input_inductance_h"])
    ...
    bridge_recommended_vrrm_v = _DEFAULT_BRIDGE_VOLTAGE_MARGIN * bridge_reverse_stress_v

    electrical_line_cycle = sample_boost_pfc_half_line_cycle(
        vac_rms_v=vac_rms_v,
        vdc_target_v=vdc_target_v,
        input_current_rms_a=electrical_i_line_rms_a,
        ripple_current_ratio=ripple_ratio,
    )
    sizing_line_cycle = sample_boost_pfc_half_line_cycle(
        vac_rms_v=vac_rms_v,
        vdc_target_v=vdc_target_v,
    ...
    inductor_requirements_h = [
        v_rec_v * duty / (max(delta_i_a, 1e-9) * fsw_hz)
        for v_rec_v, duty, delta_i_a in zip(
            sizing_line_cycle.v_rectified_v,
            sizing_line_cycle.duty,
            sizing_line_cycle.delta_i_allowed_a,
            strict=True,
        )
    ]
    total_inductance_required_h = max(inductor_requirements_h)
    worst_index = inductor_requirements_h.index(total_inductance_required_h)
    lboost_required_h = max(total_inductance_required_h - input_inductance_h, 0.0)
    delta_il_pp_target_a = ripple_ratio * sizing_i_line_peak_a
    duty_nom = min(max(1.0 - vac_peak_nom_v / max(vdc_target_v, 1e-9), 0.0), 1.0)
    total_series_inductance_h = lboost_required_h + input_inductance_h
    achieved_ripple_values_a = [
        v_rec_v * duty / max(total_series_inductance_h * fsw_hz, 1e-9)
        for v_rec_v, duty in zip(
            sizing_line_cycle.v_rectified_v,
            sizing_line_cycle.duty,
            strict=True,
        )
    ...
    delta_il_pp_nom_a = vac_peak_nom_v * duty_nom / max(total_series_inductance_h * fsw_hz, 1e-9)
    ...
    notes = [
        "First-pass CCM boost PFC synthesis using sinusoidal input-current target.",
        "Line-cycle current is sampled over a rectified half cycle; zero-crossing control is not modeled.",
        "Device selection, detailed losses, THD, EMI, and control-loop validation remain pending.",
    ]
    failure_reason = None
    if not feasible:
        failure_reason = "pfc_dc_bus_below_high_line_peak"
        notes.append(
            "Vdc target must exceed high-line peak with margin before boost PFC synthesis can be accepted."
        )

    metadata = {
        **spec.metadata,
        "implemented_stage": "first_pass_electrical_synthesis",
        "electrical_current_basis": "ideal regulated PFC power balance; Pout/(Vac_rms*1.0)",
        ...
        "sizing_efficiency_assumption_source": "explicit design input; default 0.95",
        ...
        "total_series_inductance_required_h": total_inductance_required_h,
        "input_inductance_h": input_inductance_h,
        "total_series_inductance_h": total_series_inductance_h,
        "boost_inductor_requirement_basis": "max line-cycle vrec*duty/(delta_i_allowed*fsw) minus explicit input inductance",
        "boost_inductor_worst_theta_deg": sizing_line_cycle.theta_deg[worst_index],
        "boost_inductor_worst_vrectified_v": sizing_line_cycle.v_rectified_v[worst_index],
        ...
        "i_bridge_rms_a": sizing_i_line_rms_a,
        "bridge_current_rms_a": sizing_i_line_rms_a,
        "bridge_current_peak_a": sizing_i_line_peak_a,
        "bridge_current_waveform_a": list(sizing_line_cycle.input_current_a),
        "bridge_vrrm_stress_v": bridge_reverse_stress_v,
        "bridge_reverse_stress_v": bridge_reverse_stress_v,
        "diode_reverse_stress_v": bridge_reverse_stress_v,
        "diode_voltage_margin": _DEFAULT_BRIDGE_VOLTAGE_MARGIN,
        "recommended_diode_vrrm_v": bridge_recommended_vrrm_v,
        "diode_vrrm_required_v": bridge_recommended_vrrm_v,
        "bridge_voltage_margin_policy": "boost_pfc_bridge_high_line_peak_with_2x_margin",
        "bridge_voltage_margin_basis": "high-line rectified peak input bridge reverse stress",
        "delta_il_pp_target_a": delta_il_pp_target_a,
        "delta_il_pp_nom_a": delta_il_pp_nom_a,
        "delta_il_pp_nom_basis": "line-peak Vrect*duty/((Linput+Lboost)*fsw)",
        "inductor_ripple_target_ratio": ripple_ratio,
        "inductor_ripple_target_app": delta_il_pp_target_a,
        "inductor_ripple_at_line_peak_app": delta_il_pp_nom_a,
        "inductor_ripple_worst_case_app": achieved_ripple_values_a[achieved_worst_index],
        "inductor_ripple_worst_theta_deg": sizing_line_cycle.theta_deg[achieved_worst_index],
        "inductor_ripple_evaluation_vrect_v": vac_peak_nom_v,
        "inductor_ripple_evaluation_duty": duty_nom,
        "inductor_ripple_equation": "Vrect*duty/((Linput+Lboost)*fsw)",
    }
    ...
    return TopologyCandidate(
        ...
        ccm_valid=feasible,
        mode_capable="ccm_first_pass_boost_pfc",
        ...
        notes=[
            *notes,
            "Electrical waveform comparison uses ideal regulated-PFC input power; device and magnetic sizing retain the explicit efficiency assumption.",
            "Switching ripple uses the total series inductance while candidate.inductance_h remains the designed boost inductor only.",
        ],
        metadata=metadata,
    )
